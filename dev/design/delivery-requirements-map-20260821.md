...
|---|---|---|
| **Candle** (`candle-core-fathomdb`, `candle-nn-fathomdb`, `candle-transformers-fathomdb` `=0.10.2`) | `fathomdb-embedder/Cargo.toml:35-37`; patched in root `Cargo.toml:56-60` via `[patch.crates-io]` to a pinned git rev of a FathomDB-maintained fork (`github.com/coreyt/candle-fathomdb`) | Not upstream Candle — a fork pinned to one exact commit, maintained to preserve a Linux AArch64 CPU Gemm/F16 fallback and to control CUDA linkage. The published dual-runtime Linux artifact must have no dynamic `libcudart.so.12` or driver dependency: CPU mapping/operation on a CUDA-free host is a product requirement, not an image choice. Different Cargo features compile genuinely different backends per GPU vendor (`embed-cuda`, `embed-metal`, `rerank-cuda`, `rerank-metal` — `fathomdb-embedder/Cargo.toml:96-107`), so a change here can silently affect only one platform's numerical output (see the retired "CPU↔CUDA byte-identity" ULP finding in program memory). |
