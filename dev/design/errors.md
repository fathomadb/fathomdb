...
| `ProjectionGenerationError`     | projection generation and receipt-keyed readiness  | `EngineError`     | `design/projections.md` | closed reason and field path for generation authority |
...
| `EmbedderRequired`              | pending embedding work with no configured runtime  | `EngineError`     | `design/embedder.md`    | immediate typed configuration feedback; equivalence refusal and worker failure remain operational outcomes |
...
| `ProvenanceError`               | versioned identity/provenance validation             | `EngineError` | `design/actuation.md` | closed reason plus canonical JSON-pointer field path; distinct remediation from legacy write-shape failures |
| `DependencyError`               | dependency request validation or refusal             | `EngineError` | `design/actuation.md` | closed reason plus canonical JSON-pointer field path; dependency remediation is distinct from provenance construction |
| `DependencyClosureError`        | dependency-closure keyed status validation         | `EngineError`     | `design/actuation.md` | closed reason and field path; no public administrative queue |
| `ActuationError`                | bounded actuation envelope or idempotency refusal  | `EngineError`     | `design/actuation.md` | malformed requests differ from terminal domain-refusal receipts |
| `FrozenReadError`               | frozen context authentication or state binding     | `EngineError`     | `design/retrieval.md` | stable reason and field path for tamper, foreign DB, or drift |
| `PageError`                     | frozen pagination request or continuation          | `EngineError`     | `design/retrieval.md` | bounded page remediation is distinct from generic argument failure |
| `EvidenceErrorV1`               | compact evidence search or resolution              | `EngineError`     | `design/retrieval.md` | preserves nondisclosure precedence through a closed reason |
| `DependencyTraceErrorV1`        | bounded dependency trace                           | `EngineError`     | `design/retrieval.md` | trace root, context, and work-bound failures remain distinct |
| `GraphExpansionErrorV1`         | constrained graph expansion                        | `EngineError`     | `design/retrieval.md` | closed recursive request and work-accounting failures |
| `RuntimeConfigurationError`     | process-start SQLite mode selection                | direct / `EngineOpenError` | `design/engine.md` | late/conflicting/runtime failure carries requested/effective mode |
...
| `VectorEquivalenceMismatchError`| open-time #5 probe divergence → query-time dense refusal | `EngineError` | `design/vector.md` | dense retrieval refused because the live backend's re-embedded probes diverged; text-only/FTS-only path (`search_text_only`) stays serviceable |
