...
| PARENT-01 | P1 | Parent-child LOCOMO screening: does retrieving a turn/child and returning its session/parent plus bounded neighbors improve evidence and answer context over parent-only retrieval? | Planned; TRACE-01's contract dependency is satisfied and HITL directed that it join the LOCOMO grid. Its one bounded, pre-registered treatment still requires a frozen amendment before execution. | [track plan](tracks/parent-01-parent-child-screening.md) |
| SCALE-01 | P1 | TC-5 eu7 scale fidelity: what manifest-qualified all-real CPU fidelity envelope is observed? | Planned; manifest and test-only plan-only runner preparation is integrated, and HITL has authorized smoke plus CPU characterization execution. May proceed independently of LOCOMO work. | [track plan](tracks/scale-01-tc5-fidelity.md) |
