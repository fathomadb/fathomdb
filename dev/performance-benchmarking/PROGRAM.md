...
| GRAPH-EVIDENCE-01 | implemented, smoke verified | Does frozen graph expansion disclose and resolve exact target and terminal-edge evidence without stale or unauthorized disclosure? | Native 12-cell evidence matrix and tamper refusal smoke pass; full lifecycle campaign remains deliberate. | [track plan](tracks/graph-evidence-01-fidelity.md) · [implementation plan](gauntlet-v1.2/GRAPH-BENCHMARKS-IMPLEMENTATION-PLAN.md) |
| GRAPH-EXPAND-01 | implemented, smoke verified | What correctness and performance envelope does native bounded graph expansion provide under a frozen workload? | Oracle parity, query seeds, work bounds, latency, throughput, concurrency, storage, and RSS smoke pass; larger scales remain unrun. | [track plan](tracks/graph-expand-01-bounded-traversal.md) · [implementation plan](gauntlet-v1.2/GRAPH-BENCHMARKS-IMPLEMENTATION-PLAN.md) |
| GRAPH-RETRIEVAL-01 | directional run complete | Does native bounded expansion improve labelled MuSiQue evidence retrieval over identical fused seeds? | CUDA qualification matched 300/300 historical top-10 rankings; the 300-question native run completed, with both expansion depths producing a 100% no-op rate and zero retrieval delta. | [track plan](tracks/graph-retrieval-01-native-expand.md) · [0.8.26 result](gauntlet-v1.2/GRAPH-BENCHMARKS-SMOKE-0.8.26.md) |
...
| REASON-01 | complete, rejected | Multi-hop treatment eligibility and native HippoRAG-2 comparison: can an explicit FathomDB relationship profile improve the diagnosed multi-hop gap, and how does an accepted treatment compare with the native system? | `protected_multiquery_v1` remains rejected. Compact-ledger v2 completed all 109 consumed-cohort cases and fixed the v1 abstention/continuation flaw, but lost 9.17 points groundedness and 5.50 points attribution versus A0. Close the offshoot; no confirmation, native comparison, or MEMORY-01 refresh is eligible. | [track plan](tracks/reason-01-native-hipporag2.md) · [held-out result](2026-08-30-reason-01-result.md) · [v2 result](2026-08-30-reason-01-compact-ledger-v2-result.md) · [v2 receipt](../../experiments/runs/reason-01-compact-ledger-v2-20260830T2156Z-572f51ea/record.json) |
...
for mmap128. HITL `seq-268` approved `stream_default` as the production FTS
...
value-changing extracted facts were not consolidated. LATENT-01 remains parked
until a cross-window failure set exists.
GLOBAL-01 is complete. Its source-linked lazy-coverage treatment reduced
...
REASON-01 was opened because MEMORY-01 identified a material multi-hop gap and
preserved development evidence identified a distinct, non-graph candidate. It
is now complete. `protected_multiquery_v1` improved supporting-session
...
closed incomplete. Its v2 correction completed all 109 consumed-cohort cases
and proved deterministic all-missing abstention and continuation, but failed
the descriptive no-regression boundary on groundedness and attribution. Close
the offshoot and retain A0. No untouched confirmation, native HippoRAG-2 run,
