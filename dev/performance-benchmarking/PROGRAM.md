...
| SCALE-02 | P3 | F-17 advisory scale envelope: what measured local-first range is supportable for the selected projection profile? | Compact follow-up complete; `rank_default` is the lowest-footprint eligible cell with exact 132-query equivalence. Formal 10k awaits HITL approval and production landing. | [track plan](tracks/scale-02-local-first-envelope.md) · [selection receipt](../../experiments/runs/scale-02-fts-selection-20260822T1859Z-946ebdc2/record.json) |
...
follow-up recommends `rank_default`; the formal rerun remains pending HITL and
production landing.
