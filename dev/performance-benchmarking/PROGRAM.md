...
| LOCOMO-01 | P1 | LOCOMO self-characterization: which FathomDB retrieval configuration is eligible for answer scoring? | Active; Phase-A plan-only preparation is integrated and HITL has authorized its fixed-subset/Phase-B CPU/FTS grid plus GPU/CE cells. Report factoid, temporal, and multi-session results separately. | [track plan](tracks/locomo-01-self-characterization.md) |
| PARENT-01 | P1 | Parent-child LOCOMO screening: does retrieving a turn/child and returning its session/parent plus bounded neighbors improve evidence and answer context over parent-only retrieval? | Active; HITL approved frozen `parent_child_turn_session_v1`, and its executor, external adapter, and factual qualifier are integrated. It is blocked before release on qualified unambiguous parent provenance and the LOCOMO corpus/subset prerequisites; GPU cells also require a working NVIDIA driver. Future empirical variants require a new frozen amendment and review. | [track plan](tracks/parent-01-parent-child-screening.md) |
| SCALE-01 | P1 | TC-5 eu7 scale fidelity: what manifest-qualified GPU-primary vector-stage fidelity envelope is observed? | Active; the historical CPU v1 runner, release-gated executor, and external driver are integrated. FathomDB 0.8.23 supplies the private GPU selector, but TC-5 v2 integration and review are required before it clears a run. The GPU-primary plan requires an eligible all-real source, manifest, output root, model/ground-truth/runtime qualification, one pinned RTX 3090, and a coordinator release. The 0.90 goal remains; a CPU equivalence bridge, if needed, is limited to 7,667 documents and 12 effective workers. | [track plan](tracks/scale-01-tc5-fidelity.md) · [0.8.23 amendment](2026-08-21-fathomdb-0.8.23-gpu-primary-amendment.md) |
| CORPUS-01 | P1 | Agent-memory gold coverage: are temporal change, knowledge update, supersession, and erasure represented well enough for a broad personal-memory claim? | Active; the fail-closed corpus/license matrix and human-gold protocol are integrated, and HITL has authorized corpus/license and human-gold work in parallel. No payload or human-review batch has begun: factual qualification is required first, followed by a coordinator-bound trusted amendment registry only if a qualified human-gold batch needs one. LOCOMO alone is insufficient because it has no knowledge-update class. | [track plan](tracks/corpus-01-gold-coverage.md) |
