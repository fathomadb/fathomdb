...
| SCALE-01 | P1 | TC-5 eu7 scale fidelity: what manifest-qualified all-real CPU fidelity envelope is observed? | Active; the manifest runner, release-gated executor, and external driver are integrated, and HITL has authorized smoke plus CPU characterization execution. It remains blocked on an eligible all-real source, manifest, output root, CPU/model/ground-truth/runtime qualification, and a coordinator release. The 0.90 goal remains; a separate current-configuration ground-truth remediation is authorized but requires a dated diagnostic contract. May proceed independently of LOCOMO work. | [track plan](tracks/scale-01-tc5-fidelity.md) |
