...
| GLOBAL-01 | P3 | Global sensemaking: can a bounded source-linked coverage treatment close the first run's coverage gap without losing directness, attribution, lifecycle fidelity, or acceptable cost? | V3 confirmed `finish_reason=length` at 4,096 tokens. The next correction keeps that ceiling and replaces repeated canonical reduction identifiers with short local references that the caller restores before persistence. | [track plan](tracks/global-01-native-graphrag.md) · [output-limit basis](2026-08-29-global-01-output-limit-research.md) · [v3 receipt](../../experiments/runs/global-01-lazy-witness-20260829T2118Z-b0f3c328/record.json) |
