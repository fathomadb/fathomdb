...
| GLOBAL-01 | P3 | Global sensemaking: can a bounded source-linked coverage treatment close the first run's coverage gap without losing directness, attribution, lifecycle fidelity, or acceptable cost? | Map retries now self-correct. Output-limit-aware reduction feedback is implemented and its zero-spend binding passed; resume the bound witness checkpoint, then held-out only if valid. | [track plan](tracks/global-01-native-graphrag.md) · [execution note](2026-08-29-global-01-v2-witness-execution-note.md) · [preflight receipt](../../experiments/runs/global-01-lazy-preflight-20260829T2059Z-483e11ad/record.json) |
...
correction preserves the contract; repeat zero-spend binding, then resume the
bound checkpoint. REASON-01 retains its independent native-run prerequisites.
