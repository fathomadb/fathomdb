...
| REASON-01 | complete, rejected; compact-ledger v2 preregistered | Multi-hop treatment eligibility and native HippoRAG-2 comparison: can an explicit FathomDB relationship profile improve the diagnosed multi-hop gap, and how does an accepted treatment compare with the native system? | `protected_multiquery_v1` remains rejected. V1 stopped incomplete. V2 now has deterministic all-missing abstention, exact per-cell citation schemas, fail-closed continuation, and explicit abstention grounding; its zero-spend input preflight passed, but it has not run and makes no quality claim. | [track plan](tracks/reason-01-native-hipporag2.md) · [result](2026-08-30-reason-01-result.md) · [v1 result](2026-08-30-reason-01-compact-ledger-result.md) · [v2 plan](2026-08-30-reason-01-compact-ledger-v2-plan.md) · [v2 config](../../experiments/configs/reason-01/compact-ledger-run.v2.json) |
...
closed incomplete. `protected_evidence_ledger_v2` is preregistered and passed
zero-spend input preflight; it must use a fresh v2 checkpoint and cannot create
a promotion claim from the consumed cohort. Do not start an untouched
confirmation until a complete v2 diagnostic supports a separate plan.
