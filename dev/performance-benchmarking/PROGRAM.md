...
| GLOBAL-01 | P3 | Global sensemaking: can a bounded source-linked coverage treatment close the first run's coverage gap without losing directness, attribution, lifecycle fidelity, or acceptable cost? | Validation-aware retries are implemented and the fresh authorized zero-spend preflight passed. Run A/A and the witness from a new root; run held-out only if valid. | [track plan](tracks/global-01-native-graphrag.md) · [execution note](2026-08-29-global-01-v2-witness-execution-note.md) · [preflight receipt](../../experiments/runs/global-01-lazy-preflight-20260829T2053Z-483e11ad/record.json) |
...
zero-spend preflight passed. Run a fresh witness root under the existing
authorization. REASON-01 retains its independent native-run prerequisites.
