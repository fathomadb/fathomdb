...
| SCALE-02 | P3 | F-17 advisory scale envelope: what measured local-first range is supportable for the selected projection profile? | Active with A0 and the SCALE-01 primary; pre-register the workload matrix before execution. Claim policy remains open pending an HITL proposal; measure canonical records and derived projection rows separately. | [track plan](tracks/scale-02-local-first-envelope.md) |
