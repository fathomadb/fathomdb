...
| TEMPORAL-01 | P3 | Time-scoped retrieval: do temporal filters and version-aware projections return the correct state without stale superseded evidence? | Factual inputs confirmed. The official releases lack an external validity-window manifest, so the first cell is synthetic TRACE validity; external-corpus comparison remains blocked on a reviewed manifest and adapter. | [track plan](tracks/temporal-01-time-scoped-retrieval.md) · [preflight](2026-08-23-temporal-01-factual-preflight-result.md) · [source review](2026-08-23-temporal-01-source-manifest-review.md) |
...
TEMPORAL-01 runs synthetic TRACE validity first; its corpus comparison remains
blocked on a reviewed manifest and adapter. EXTRACT-01 next needs its factual
