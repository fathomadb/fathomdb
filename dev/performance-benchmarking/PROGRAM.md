...
| GLOBAL-01 | P3 | Global sensemaking: can a bounded source-linked coverage treatment close the first run's coverage gap without losing directness, attribution, lifecycle fidelity, or acceptable cost? | V3 uses a bounded 4,096-token matched reduction ceiling. Its first paid call exposed a local finish-reason accounting bug before checkpointing; the fix is test-backed and requires a fresh preflight and root. | [track plan](tracks/global-01-native-graphrag.md) · [output-limit basis](2026-08-29-global-01-output-limit-research.md) |
