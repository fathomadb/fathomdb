...
1. Repeat v3 zero-spend preflight after the finish-reason accounting fix.
2. Run fresh A/A and the three-question witness from a new root.
3. Run held-out only if the v3 witness is valid.
...
| [GLOBAL-01](tracks/global-01-native-graphrag.md) | V3 accounting fix ready for preflight | The first paid response exposed a local mixed-type usage check before checkpointing. The test-backed fix keeps finish reason separate from numeric token validation; rebind before a fresh root. |
