...
1. **TEMPORAL-01 / EXTRACT-01:** write factual preflights and fixed measurement
   contracts using the registered TimeQA, evaluation-only TimelineQA, and
   LongMemEval inputs. Retain the current limited supersession claim boundary.
...
| [TEMPORAL-01](tracks/temporal-01-time-scoped-retrieval.md) | Input acquisition complete | Bind TimeQA, evaluation-only TimelineQA, and LongMemEval-S to a factual preflight and fixed comparison contract; report each corpus separately. |
