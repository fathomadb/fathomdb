...
the individual [track charters](tracks/README.md).
...
| Track worker | One charter, one isolated worktree, its tests, implementation, and a commit | Change another track's charter, shared integration files, or run external/costly work without authorization |
| Independent reviewer | Read-only review of one worker diff against its charter | Edit the worker checkout or waive missing evidence |
...
reads the live coordinator board and `brief` reads the charter; neither
authorizes a benchmark run. The worker then records a compact handoff packet
containing:

1. Track ID, charter path, dedicated-worktree base SHA, and owned paths.
...
their charters; Track Runner governs how any non-historical work on them moves.
...
2. A different, read-only Codex reviewer checks the diff against the charter,
...
3. The coordinator integrates only an accepted commit, verifies it from Git,
   ...
   PROGRAM/charter state where the status actually changed.
...
charter only when durable state has changed; leave historical result notes
historical.
