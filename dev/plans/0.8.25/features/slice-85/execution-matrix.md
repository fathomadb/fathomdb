...
| Real corpus fidelity — `tc5-bridge` | Decision `seq-31` assigns grown-corpus fidelity to TC-5. Run only the frozen 7,667-document, 100-query TC-5 bridge against exact candidate wheel/CLI/private-binary bytes. Require the registered fixture, ground-truth and SUT-result digests; retain the 17,272-document primary without rerunning it. |
...
completed TC-5 track supersede only that cell. The candidate-bound TC-5 bridge
is therefore the sole remaining local fidelity action.
...
| TC-5 AC-075 bridge | Build `fathomdb-tc5-benchmark` with `tc5-benchmark-cuda`; generate `$RUN_DIR/tc5-candidate-config.json` from the frozen TC-5 v2 pins with release/version `0.8.25`, `FINAL_SHA`, the retained input/model digests, local registered GPU UUID, and exact wheel/CLI/private-binary paths and hashes; run `python3 -m experiments.tc5_gpu_v2 dry-run --config $RUN_DIR/tc5-candidate-config.json --arm bridge --output-root $RUN_DIR/tc5-bridge`, then the corresponding `run ... --binary $RUN_DIR/artifacts/fathomdb-tc5-benchmark` | 7,200 s; 7,667 real documents, 100 complete queries, zero synthetic rows, fixture `9e92d236…e622ed`, ground truth `ef6be77b…f34b`, SUT result `436493dc…f84a`, recall `0.958`, and candidate/artifact identity match |
