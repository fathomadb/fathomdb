...
| Jetson — `jetson-tegra` | Over direct SSH, check out the exact candidate, build and clean-install the `0.8.25+tegra` Python wheel, then require `cpu`, `auto` and `cuda:0` smokes plus the registered-GPU witness. Retain a hashed local receipt. The Jetson remains a self-hosted runner for later use; GitHub upload is not a closure requirement. Do not add unsupported Tegra N-API or Windows CUDA cells. |
...
Final disposition: the last Windows verification correction touched the broad
Engine-source input set even though it is confined to private `test-hooks`.
The conservative invalidation policy therefore converts the performance row
from reuse to the single bounded AC-012 rerun above. It passed 1,000 measured
queries at p50 1 ms and p99 3 ms; no wider performance campaign is implied.
...
in `slice75-closure-manifest.json` except: replace `performance` with the
bounded AC-012 command above when its broad input digest is invalidated; add current runtime-configuration
