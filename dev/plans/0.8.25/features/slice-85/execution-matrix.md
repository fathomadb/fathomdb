...
| Real corpus fidelity — `tc5-bridge` | Decision `seq-31` assigns grown-corpus fidelity to TC-5; the later direct-HITL amendment makes TC-5 GPU-primary. Repeat only the registered 7,667-document, 100-query GPU smoke against exact candidate wheel/CLI/private-binary bytes. This is not the optional CPU equivalence bridge. Require the registered fixture, ground-truth and SUT-result digests; retain the 17,272-document primary without rerunning it. |
