...
actionlint, lychee, MkDocs, maturin, gh and virsh. Python 3.10/3.11 and a usable
NVIDIA driver are absent locally, so those obligations need remote executors.
...
| Runtime configuration — added after Slice 75 | Run the four exact source commands and installed-artifact command in the sealed-routes section below. Require 7/4/3/3 source tests and six installed fresh-process cases. |
...
| Protected writes / CE — retained `slice71-write`, `slice72` | Rerun the six Slice 79 candidate-only 71B cells and the Slice 72 candidate CPU/CUDA installed CE profile. Slice 80 invalidated their broad Engine-source sets. Reuse the registered historical baselines only; never rerun experiment matrices. |
...
| Runtime Python source | `python -m pytest src/python/tests/test_slice79_runtime_configuration.py -q` | 300 s; 3 passed |
| Runtime TypeScript source | `npm run build:debug --prefix src/ts` then `node --test src/ts/dist/tests/slice79-runtime-configuration.test.js` | 900 s; 3 passed |
