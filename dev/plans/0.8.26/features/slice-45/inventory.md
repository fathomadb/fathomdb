...
| One writer owns mutation transactions; reader-pool transactions own frozen search, graph selection, and evidence materialization | accepted single-writer/frozen-read/evidence ADRs; current interfaces | engine writer path; `actuation.rs`, `frozen_read.rs`, `graph_expand.rs`, `evidence.rs` | Slice 20 and Slice 35/40 suites/status records |
