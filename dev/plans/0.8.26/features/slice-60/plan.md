...
| R26-60K: isolation and regression boundary | AC26-60K: concurrent ownership and a busy SQLite checkpoint use retryable exit 71; nonempty rollback journal, missing/zero-length/noncurrent/main-corrupt database use unrecoverable exit 70; none claims recovery or changes protected bytes. No recovery path becomes default-SDK reachable, other actions keep generic admitted-Engine dispatch, and focused CLI/engine tests plus the canonical gate pass. |
