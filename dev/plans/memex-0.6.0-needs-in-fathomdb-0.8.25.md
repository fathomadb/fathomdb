...
| 12 | Retrieval explanation | Query traces, per-hit ranks/scores, soft fallback, projection cursor, and privacy-safe query IDs exist. | **Partial.** Add compact dependency/graph origin and inclusion/degradation state in 0.8.25; expanded exclusion and path replay move to 0.8.28. | 50/55/60 core; 0.8.28 extension |
...
| 17 | Atomic consolidation application | `consolidate_with_provider` lets FathomDB invoke a caller provider and then mutate state. | **Missing model-free actuation.** Apply the lifecycle/write subset of an already-decided verdict through the 0.8.25 core batch; broaden unsupported operation forms in 0.8.26. | 25 core; 0.8.26 extension |
...
| 19 | Operational tracing | Search explanation, source IDs, projection status, telemetry correlation, and orphan-provenance doctor checks exist. | **Partial.** Add bounded reciprocal traversal and compact inclusion/degradation explanation in 0.8.25; expanded exclusion/correlation moves to 0.8.28. | 55 core; 0.8.28 extension |
...
| # | Need | Existing FathomDB mechanism | Gap and 0.8.25 disposition | Slice |
| ---: | --- | --- | --- | ---: |
| 21 | Concurrent read/write guarantees | WAL, reader workers, deferred reader transactions, stress tests, and typed storage/runtime failures exist. | **Partial for new contracts.** Define optional frozen-read behavior in Slice 35 and measure representative cold/steady contention in Slice 75; full lease behavior moves to 0.8.27. | 35/75 core; 0.8.27 extension |
| 22 | Predictable local performance | Accepted FTS envelope reaches 50k with exact top-100 equivalence; graph traversal is bounded. | **Partial for new contracts.** Measure representative evidence, pagination, dependency, readiness, and erasure regressions; exhaustive matrices remain experimental. | 75 core; review 0.8.33 |
...
| 24 | Data-plane evaluation support | Typed experiment receipts and the completed performance program exist. | **Partial.** Slice 10 makes classification executable with a native fixture; Slice 75 records the final packaged-candidate `Engine.search` witness. | 10, 75 |
