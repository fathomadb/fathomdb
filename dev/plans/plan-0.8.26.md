...
| ---: | --- | --- | --- |
| 0 | Record environment/project-infrastructure needs and establish the isolated release workspace and draft plan. | 0.8.25 + Memex scaffold | Landed on `origin/main` |
| 1 | Perform a read-only dependency, advisory, Dependabot, and pinning sweep; propose responses without upgrades. | 0 | Landed on `origin/main` |
| 2 | Review repository cruft and propose keep, deprecate-in-place, archive-in-place, or delete. | 1 | Landed on `origin/main` |
| 3 | Draft user-need, requirement, acceptance, interface, ADR, and architecture CRUD; allocate every draft to one slice. | 2 | Landed on `origin/main` |
| 4 | Review the proposed architecture and high-level code alignment; propose corrections only. | 3 | Landed on `origin/main` |
| 5 | Review verification adequacy from need through test and critical path. | 4 | Landed on `origin/main` |
| 6 | Review local build, preflight, transcript, and `agent-verify` failure evidence; propose pragmatic corrections without implementing them. | 5 | Landed on `origin/main` |
| 7 | Review post-build CI/CD, packaging, gitleaks, and registry failure evidence; propose pragmatic corrections and delivery placement. | 6 | Landed on `origin/main` |
| 8 | Score all proposals, conduct interactive HITL decisions, replace/review Slice 9, and update this plan. | 7 | Landed on `origin/main`; D26-01 later resolved at `seq-290` |
| 9 | Implement only HITL-approved repository preparation under the reviewed Slice 9 plan. | 8 | Landed on `origin/main` at `ded6c72f` |
| 10 | Repair frozen explanation, complete public guidance, and add the installed-artifact conformance witness. | 9 | Landed on `origin/main` at `c2795caf` |
| 15 | Run the bounded graph-evidence performance and erasure-linearization implementation spike; accept or narrow the Slice 20 design. | 10 | Landed on `origin/main`; opt-in V1 sidecar selected at `seq-290` |
| 20 | Add immutable-revision graph-target and terminal-edge evidence resolution under frozen authority. | 15 | Landed on `origin/main` at `d335ae9a` |
| 30 | Qualify and harden the existing versioned read-only operator integrity inspection route. | 20 | Landed on `origin/main` at `6d80e7a8` |
| 35 | Prove the breaking changed-in-place V1 actuation, fresh-database, receipt, and performance contract; accept or narrow the Slice 40 design. | 30 | Landed on `origin/main` at `9a81a75c` |
| 40 | Extend V1 actuation in place with the accepted atomic derived-edge contract and fresh-database boundary. | 35 | Landed on `origin/main` at `bfb2132b` |
| 45 | Reconcile maintained architecture documentation with accepted decisions, public contracts, and the as-built 0.8.26 system. | 40 | Landed on `origin/main` at `6f68e2fd` |
| 46 | Reconcile and organize maintained technical design documentation under the Slice 45 architecture hierarchy. | 45 | Landed on `origin/main` at `6d952c9c` after release-wide review and full-gate remediation |
| 50 | Run integrated Memex-profile, cross-SDK, platform, package, documentation, and non-publishing release verification. | 46 | Landed on `origin/main` at `68514f70`; superseded as the final candidate by the reopened ladder |
| 55 | Replace subset surface checks with an executable canonical-operation Python/TypeScript parity oracle. | 50 | Landed on `origin/main` at `a5859cd9` |
| 60 | Reconcile the maintained retrieval, recovery, and engine owners against one code-grounded authority matrix, then back-check earlier owner and SDK artifacts. | 55 | Landed on `origin/main` at `173c49cb` |
| 65 | Enforce maintained-owner authority topology, reconcile error ownership, rerun affected earlier proofs, and bind the replacement exact candidate. | 60 | Landed on `origin/main` at `8ffb3486`; closeout at `cad95b4d` |
