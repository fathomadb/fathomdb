...
| 65 | WAL-ATTRIBUTION — Windows WAL checkpoint root-cause attribution | IN_PROGRESS / UNATTRIBUTED — runs `32014441608`/job `95340828268` and `32015551136`/job `95344203972` add two clean hosted unperturbed paired actual-checkpoint repetitions for direct Rust and current installed Python: writer autocommit, complete 12-role direct autocommit inventory, idle collector, no local overlap, and zero busy/frame counters. Job `95332307148` remains contrary variable evidence: five unattributed BUSY/25-frame current-Python attempts after idle recovery reads; its earlier raw ladder was perturbing. Exactly one fresh Windows repetition still blocks completion, landing, attribution, and remedies. |
