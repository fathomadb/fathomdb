...
def profile_environment(arm: str, ready: Path, go: Path) -> dict[str, str]:
...
def cmd_profile(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="fathomdb-slice76-profile-") as temp:
        ...
        ready = root / "ready"
        go = root / "go"
        environment = profile_environment(args.arm, ready, go)
        command = [
            str(binary),
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if actual_sha != args.expected_sha:
        raise ValueError(f"wrong binary identity: expected {args.expected_sha}, got {actual_sha}")
    environment = clean_environment()
