...
# owner-approved 0.8.25 Slice 20/25/30/35/40/45/50/55 additions — pinned to the exact content of
# src/conformance/governed-surface-allowlist.json at the provenance commit
# recorded in the pin (66 allowlist members, 5 core, recovery_denylist unchanged
...
expect_out '66 allowlist / 5 core / 5 recovery_denylist' \
...
# The accumulated signed/approved surface is an exact 66-member shape, not
...
# signed and approved surface through 0.8.25 Slice 55.
set +e
SLICE55_SHAPE="$(python3 - "$REAL_FILE" "$REAL_PIN" <<'PY'
import json
import sys
...
expected_allowlist = [
    ...
    "read.projectionGenerationStatus",
    "read.mutation_projection_status",
    "read.mutationProjectionStatus",
    "read.canonical_page",
    "read.canonicalPage",
    "read.operational_state",
    "read.operationalState",
    "read.operational_state_page",
    "read.operationalStatePage",
]
expected_counts = {"allowlist": 66, "core": 5, "recovery_denylist": 5}
...
expected_0825_comment = "RE-ISSUED 2026-09-04 (HITL-approved 0.8.25 Slice 20 and Slice 25 plans)."
expected_slice30_provenance = "approved 0.8.25 Slice 30"
expected_slice30_comment = "RE-ISSUED 2026-09-04 (HITL-approved 0.8.25 Slice 30 plan and execution)."
expected_slice35_provenance = "approved 0.8.25 Slice 35"
expected_slice35_comment = "RE-ISSUED 2026-09-04 (HITL-approved 0.8.25 Slice 35 plan and execution)."
expected_slice40_provenance = "approved 0.8.25 Slice 40"
expected_slice40_comment = "RE-ISSUED 2026-09-05 (HITL-approved 0.8.25 Slice 40 plan and execution)."
expected_slice45_provenance = "approved 0.8.25 Slice 45"
expected_slice45_comment = "RE-ISSUED 2026-09-05 (HITL-approved 0.8.25 Slice 45 plan and execution)."
expected_slice50_provenance = "approved 0.8.25 Slice 50"
expected_slice50_comment = "RE-ISSUED 2026-09-05 (HITL-approved 0.8.25 Slice 50 plan and execution)."
expected_slice55_provenance = "approved 0.8.25 Slice 55"
...
if allowlist.get("allowlist") != expected_allowlist:
    problems.append("allowlist is not the exact approved 66-member accumulated shape")
if pin.get("allowlist") != expected_allowlist:
    problems.append("pin allowlist is not the exact approved 66-member accumulated shape")
...
if expected_0825_provenance not in pin.get("ac", ""):
    problems.append("pin lacks approved 0.8.25 Slice 20/25 provenance in its ac field")
if expected_0825_comment not in "\n".join(pin.get("_comment", [])):
    problems.append("pin lacks approved 0.8.25 Slice 20/25 provenance in its comment")
if expected_slice30_provenance not in pin.get("ac", ""):
    problems.append("pin lacks approved 0.8.25 Slice 30 provenance in its ac field")
if expected_slice30_comment not in "\n".join(pin.get("_comment", [])):
    problems.append("pin lacks approved 0.8.25 Slice 30 provenance in its comment")
if expected_slice35_provenance not in pin.get("ac", ""):
    problems.append("pin lacks approved 0.8.25 Slice 35 provenance in its ac field")
if expected_slice35_comment not in "\n".join(pin.get("_comment", [])):
    problems.append("pin lacks approved 0.8.25 Slice 35 provenance in its comment")
if expected_slice40_provenance not in pin.get("ac", ""):
    problems.append("pin lacks approved 0.8.25 Slice 40 provenance in its ac field")
if expected_slice40_comment not in "\n".join(pin.get("_comment", [])):
    problems.append("pin lacks approved 0.8.25 Slice 40 provenance in its comment")
if expected_slice45_provenance not in pin.get("ac", ""):
    problems.append("pin lacks approved 0.8.25 Slice 45 provenance in its ac field")
if expected_slice45_comment not in "\n".join(pin.get("_comment", [])):
    problems.append("pin lacks approved 0.8.25 Slice 45 provenance in its comment")
if expected_slice50_provenance not in pin.get("ac", ""):
    problems.append("pin lacks approved 0.8.25 Slice 50 provenance in its ac field")
if expected_slice50_comment not in "\n".join(pin.get("_comment", [])):
    problems.append("pin lacks approved 0.8.25 Slice 50 provenance in its comment")
if expected_slice55_provenance not in pin.get("ac", ""):
    problems.append("pin lacks approved 0.8.25 Slice 55 provenance in its ac field")
if expected_slice55_comment not in "\n".join(pin.get("_comment", [])):
    problems.append("pin lacks approved 0.8.25 Slice 55 provenance in its comment")

if problems:
    raise SystemExit("; ".join(problems))
print("exact approved 66-member accumulated shape and explicit provenance")
PY
)"
SLICE55_SHAPE_RC=$?
set -e
if [ "$SLICE55_SHAPE_RC" -eq 0 ]; then
  pass "$SLICE55_SHAPE"
else
  fail "the pin must retain the exact approved 66-member accumulated shape and explicit provenance: $SLICE55_SHAPE"
fi
...
# ======================= Arm 1 (RED): added member (67) =======================
F="$(copy_file added-member)"
mutate "$F" 'd["allowlist"].append("shiny.new_verb")'
check_fixture "$F"
expect_rc 1 "an ADDED allowlist member (67) HARD-fails"
...
expect_out 'Pinned 66 member\(s\), on disk 67' "added-member states pinned-vs-on-disk counts"
...
# ====================== Arm 2 (RED): removed member (65) ======================
F="$(copy_file removed-member)"
mutate "$F" 'd["allowlist"].remove("purge")'
check_fixture "$F"
expect_rc 1 "a REMOVED allowlist member (65) HARD-fails"
expect_out 'REMOVED purge' "removed-member NAMES the member that vanished"
expect_out 'Pinned 66 member\(s\), on disk 65' "removed-member states pinned-vs-on-disk counts"
...
expect_out 'counts block says 66' "lazy-repin is also caught by the counts assertion"
...
# agreed with the pin, and the one check that still knew the signed size
# had been quietly switched off. Pre-fix this exited 0 and printed
#   "ok ... (None allowlist / 5 core / 5 recovery_denylist)"
# for a surface carrying one additional `smuggled.verb` member.
