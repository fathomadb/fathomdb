...
# owner-approved 0.8.25 Slice 20/25/30/35/40/45/50 additions — pinned to the exact content of
# src/conformance/governed-surface-allowlist.json at the provenance commit
# recorded in the pin (64 allowlist members, 5 core, recovery_denylist unchanged
...
expect_out '64 allowlist / 5 core / 5 recovery_denylist' \
...
# The accumulated signed/approved surface is an exact 64-member shape, not
...
# signed and approved surface through 0.8.25 Slice 50.
set +e
...
expected_counts = {"allowlist": 64, "core": 5, "recovery_denylist": 5}
...
if allowlist.get("allowlist") != expected_allowlist:
    problems.append("allowlist is not the exact approved 64-member accumulated shape")
if pin.get("allowlist") != expected_allowlist:
    problems.append("pin allowlist is not the exact approved 64-member accumulated shape")
...
print("exact approved 64-member accumulated shape and explicit provenance")
PY
)"
SLICE50_SHAPE_RC=$?
set -e
if [ "$SLICE50_SHAPE_RC" -eq 0 ]; then
  ...
  fail "the pin must retain the exact approved 64-member accumulated shape and explicit provenance: $SLICE50_SHAPE"
fi
...
# ======================= Arm 1 (RED): added member (65) =======================
F="$(copy_file added-member)"
mutate "$F" 'd["allowlist"].append("shiny.new_verb")'
check_fixture "$F"
expect_rc 1 "an ADDED allowlist member (65) HARD-fails"
...
expect_out 'Pinned 64 member\(s\), on disk 65' "added-member states pinned-vs-on-disk counts"
...
# ====================== Arm 2 (RED): removed member (63) ======================
F="$(copy_file removed-member)"
mutate "$F" 'd["allowlist"].remove("purge")'
check_fixture "$F"
expect_rc 1 "a REMOVED allowlist member (63) HARD-fails"
expect_out 'REMOVED purge' "removed-member NAMES the member that vanished"
expect_out 'Pinned 64 member\(s\), on disk 63' "removed-member states pinned-vs-on-disk counts"
...
expect_out 'counts block says 64' "lazy-repin is also caught by the counts assertion"
