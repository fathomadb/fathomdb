...
# owner-approved 0.8.25 Slice 20/25 additions — pinned to the exact content of
# src/conformance/governed-surface-allowlist.json at the provenance commit
# recorded in the pin (42 allowlist members, 5 core, recovery_denylist unchanged
...
expect_out '42 allowlist / 5 core / 5 recovery_denylist' \
...
# The accumulated signed/approved surface is an exact 42-member shape, not
...
# plus the approved 0.8.25 Slice 20/25 additions.
set +e
...
expected_counts = {"allowlist": 42, "core": 5, "recovery_denylist": 5}
...
if allowlist.get("allowlist") != expected_allowlist:
    problems.append("allowlist is not the exact approved 42-member accumulated shape")
if pin.get("allowlist") != expected_allowlist:
    problems.append("pin allowlist is not the exact approved 42-member accumulated shape")
...
print("exact approved 42-member accumulated shape and explicit provenance")
PY
)"
SLICE30_SHAPE_RC=$?
set -e
if [ "$SLICE30_SHAPE_RC" -eq 0 ]; then
  ...
  fail "the pin must retain the exact approved 42-member accumulated shape and explicit provenance: $SLICE30_SHAPE"
fi
...
expect_out 'Pinned 42 member\(s\), on disk 43' "added-member states pinned-vs-on-disk counts"
...
expect_out 'Pinned 42 member\(s\), on disk 41' "removed-member states pinned-vs-on-disk counts"
...
expect_out 'counts block says 42' "lazy-repin is also caught by the counts assertion"
