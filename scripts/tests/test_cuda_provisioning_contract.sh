...
path = Path(sys.argv[1])
text = path.read_text()
needle = "NVCC_CCBIN=/opt/rh/gcc-toolset-13/root/usr/bin/g++"
if text.count(needle) != 1:
    raise SystemExit("fixture no longer contains exactly one NVCC GCC 13 selection")
path.write_text(text.replace(needle, "NVCC_CCBIN=/usr/bin/g++", 1))
PY
