...
assert_contains '[ -x "$HOME/.cargo/bin/cargo" ]' "workflow fails closed when the runner Cargo toolchain is absent"
assert_contains 'printf '''%s\n''' "$HOME/.cargo/bin" >> "$GITHUB_PATH"' "workflow exposes the runner Cargo toolchain to every build step"
