...
require "$SLICE70" '`--pax-option=delete=atime,delete=ctime`'
require "$SLICE70" '`gzip -n`'
require "$SLICE70" 'doctor gpu --json'
require "$SLICE70" 'CLI evidence is diagnostic-only'
require "$SLICE70" 'Python and N-API installed-artifact smokes are the only model-operation proof'
...
require "$SLICE70" 'deterministic `FixtureCudaProvider` inventory/probe fixtures'
...
require "$SLICE10" 'fresh writable `XDG_CACHE_HOME=/fathomdb-product-cache`'
require "$SLICE10" 'forced-cuda-unavailable-python-stdout.txt'
require "$SLICE10" '`stdout_filename`'
require "$SLICE10" 'root `evidence_sha256` map binds each named capture'
...
require "$SLICE20" 'CLI evidence is diagnostic-only'
require "$SLICE20" 'fresh writable `XDG_CACHE_HOME=/fathomdb-product-cache`'
...
if [[ "${SLICE70_SKIP_MUTATIONS:-0}" != 1 ]]; then
  ...
  assert_mutation_rejected diagnostic-only 0.8.23-slice-70-dual-runtime-device-policy.md \
    's/CLI evidence is diagnostic-only/CLI evidence is model-operation proof/'
