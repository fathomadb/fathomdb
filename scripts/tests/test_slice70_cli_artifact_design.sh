...
require "$SLICE70" 'fathomdb-cli` feature is exactly `embed-cuda = ["default-embedder", "fathomdb/embed-cuda", "fathomdb-embedder/embed-cuda"]`'
...
require "$SLICE20" 'exactly four package files'
...
require "$SLICE20" 'archive has exactly one POSIX root directory'
...
require "$PLAN" 'No workflow, release helper, or verifier implementation belongs to this design-only step.'
