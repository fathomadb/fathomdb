...
if [ -z "$default_embedder_block" ]; then
  ...
  if grep -q 'actions/setup-node@48b55a011bda9f5d6aeb4c2d9c7362e8dae4041e' <<<"$default_embedder_block" && \
    grep -qE '^[[:space:]]*node-version: "24\.19\.0"[[:space:]]*$' <<<"$default_embedder_block"; then
    pass "default-embedder CI job pins primary Node 24.19.0"
  else
    fail "default-embedder CI job must set up pinned Node 24.19.0"
  fi
