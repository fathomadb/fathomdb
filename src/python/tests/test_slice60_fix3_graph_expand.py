"""Slice 60 FIX-3 RED: invalid graph context carriers fail before native I/O."""
...
def _request(sdk_types):
    return sdk_types.GraphExpandRequestV1(
        schema_version=1,
        seed=sdk_types.GraphExplicitSeedV1(
            schema_version=1, type="explicit", logical_ids=(sdk_types.IdSpace("logical", "root"),)
        ),
        direction="outgoing",
        edge_kinds=(),
        target_kinds=(),
...
def test_invalid_context_carrier_is_local_type_error_without_native_call(monkeypatch) -> None:
    graph, sdk_types = _load_graph(monkeypatch)
    request = _request(sdk_types)
    object.__setattr__(request, "context", object())
    calls: list[str] = []

    class Native:
        def graph_expand(self, wire: str) -> str:
            calls.append(wire)
            raise AssertionError("invalid context must not reach native")

    with pytest.raises(
        TypeError, match="graph.expand context must be a GraphReadContextV1 carrier"
    ):
        graph.expand(types.SimpleNamespace(_native=Native()), request)
    assert calls == []
