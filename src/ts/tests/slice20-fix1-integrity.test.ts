...
test("consistent invalid stored source version maps to StorageError", async () => {
  ...
  try {
    db.exec("UPDATE _fathomdb_source_versions SET source_version_id='bad version'");
    db.exec("UPDATE _fathomdb_source_links SET source_version_id='bad version'");
  } finally {
    db.close();
  }
