...
def _write_inputs(tmp_path: Path, *, question_count: int = 32) -> dict[str, str]:
    questions = [
        {"question": f"question {index}", "evidence": ["turn-1"], "category": (index % 3) + 1}
        for index in range(question_count)
    ]
    corpus = [{"conversation": {"speaker_a": "Ada", "session_1": [
        {"speaker": "Ada", "text": "raw corpus text must not leak", "dia_id": "turn-1"},
    ]}, "qa": questions}]
    ...
    subset_path.write_text(json.dumps({"schema_version": "locomo-fixed-subset.v1", "question_ids": [
        f"locomo-0-q-{index}" for index in range(question_count)
    ]}), encoding="utf-8")
    trace_path = tmp_path / "trace.json"
    trace_path.write_text(json.dumps(adapter.synthetic_trace_sidecar("source-1")), encoding="utf-8")
    relation_path = tmp_path / "relations.json"
    relation_path.write_text(json.dumps(adapter.relation_proof_document(
        turns, sessions, child_id="turn-1", session_id="session_1", source_id="source-1",
    )), encoding="utf-8")
    return {
        "corpus": str(corpus_path), "turn_provenance": str(turn_path),
        "session_provenance": str(session_path), "dry_run_subset": str(subset_path),
        "trace_projection": str(trace_path), "parent_relation_proof": str(relation_path),
    }


def _request(tmp_path: Path, *, device: str = "cpu", treatment: str = "fts_only") -> dict[str, object]:
    return {
        "schema_version": "locomo-live-executor.request.v1", "release_id": "release-1",
        "action": "fixed_subset_dry_run", "mode": "dry_run",
        "cell": {
            "cell_id": f"turn--{treatment}--{device}--cold", "program_track": "LOCOMO-01",
            "ingest_unit": "turn", "treatment": treatment,
            "retrieval": "fts" if treatment.startswith("fts") else "hybrid",
            "runtime": {"device": device, "cache_state": "cold"}, "parent_child": None,
        },
        "external_inputs": _write_inputs(tmp_path), "output_root": str(tmp_path / "output"),
    }
...
def test_adapter_emits_complete_safe_cell_result_and_uses_the_fts_engine_path(tmp_path):
    engine = _Engine()
    result = adapter.execute_request(_request(tmp_path), engine_factory=lambda _path, _dense: engine)
...
def test_adapter_rejects_unknown_request_fields_before_loading_external_inputs(tmp_path):
...
def test_gpu_cell_never_silently_uses_cpu_when_cuda_attestation_fails(tmp_path, monkeypatch):
    request = _request(tmp_path, device="gpu", treatment="hybrid")
    monkeypatch.setattr(adapter, "_require_single_visible_cuda", lambda: (_ for _ in ()).throw(
...
def test_adapter_stdout_parser_rejects_duplicate_keys_and_parent_requires_relation_proof(tmp_path):
    ...
    Path(request["external_inputs"]["parent_relation_proof"]).write_text("{}", encoding="utf-8")
